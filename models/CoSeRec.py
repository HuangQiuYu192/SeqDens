...
class CoSeRec(SequentialRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.similarity_model_name = config["similarity_model_name"]
        ...
        self.dataset_file = os.path.join(config['data_path'], config['dataset'] + '.inter')
        self.similarity_model_path = os.path.join(config['data_path'], self.similarity_model_name + '_similarity.pkl')

        self.offline_similarity_model = OfflineItemSimilarity(data_file=self.dataset_file,
...
def _load_user_sequences_from_interactions(data_file):
    """
    Read an interaction log and convert it into user sequences.

    The input file is expected to contain a header and at least four columns:
    user id, item id, rating, and timestamp. Interactions are grouped by user
    and sorted by timestamp before the last three items are held out from the
    training sequence, matching the original sequence-based behavior.
    """
    inter_df = pd.read_csv(data_file, sep=r"\s+", engine="python")
    if inter_df.shape[1] < 4:
        raise ValueError("interaction file must contain at least four columns")

    user_col = inter_df.columns[0]
    item_col = inter_df.columns[1]
    time_col = inter_df.columns[3]

    inter_df = inter_df[[user_col, item_col, time_col]].copy()
    inter_df[user_col] = inter_df[user_col].astype(str)
    inter_df[item_col] = inter_df[item_col].astype(str)
    inter_df[time_col] = pd.to_numeric(inter_df[time_col], errors="coerce")
    inter_df = inter_df.dropna(subset=[time_col])
    inter_df = inter_df.sort_values([user_col, time_col], kind="mergesort")

    train_data = []
    train_data_list = []
    train_data_set_list = []

    for userid, user_interactions in inter_df.groupby(user_col, sort=False):
        items = user_interactions[item_col].tolist()
        if len(items) <= 3:
            continue
        items = items[:-3]
    ...
    return train_data_list, set(train_data_set_list), train_data
...
class OfflineItemSimilarity:
    ...
    def _load_train_data(self, data_file=None):
        """
        Read the interaction log, group it into per-user sequences, and keep
        the original training-only slicing behavior.
        """
        train_data_list, train_data_set_list, train_data = _load_user_sequences_from_interactions(data_file)
        return train_data_list, train_data_set_list, self._convert_data_to_dict(train_data)

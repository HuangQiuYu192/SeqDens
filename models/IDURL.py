...
class IDURL(SequentialRecommender):
    def __init__(self, config, dataset):
        super(IDURL, self).__init__(config, dataset)
    ...
    def forward(self, item_seq, item_seq_len, return_all_layers=False):
        ...
        seq_output = self.gather_indexes(trm_output[-1], item_seq_len - 1)
    ...
    def calculate_loss_prob(self, interaction):
        ...
        ori_u_repr = seq_output.unsqueeze(dim=1)  # [B,D] -> [B,1,D]
        ...
        if self.idra == 1 and self.training:
            ...
            su_aug_seq_output = self.forward(su_aug_seq, su_aug_lengths)
            su_aug_u_repr = su_aug_seq_output.unsqueeze(dim=1)  # [B,D] -> [B,1,D]
            ...
            # cl task & cl loss
            loss += self.align_lambda * self.cl_task(un_aug_repr, su_aug_repr, temp=self.tau, batch_size=item_seq_len.shape[0])

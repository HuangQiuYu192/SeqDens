...
class DuoRec(SequentialRecommender):
    ...
    def calculate_loss(self, interaction):
        ...
        cl_loss = self.cal_cl_loss(interaction, seq_output)
        return loss + cl_loss
        

    def cal_cl_loss(self, interaction, seq_output):
        item_seq = interaction[self.ITEM_SEQ]
        item_seq_len = interaction[self.ITEM_SEQ_LEN]
        # Unsupervised NCE
        ...
        if self.ssl in ['us', 'su']:
            sem_aug, sem_aug_lengths = interaction['item_id_aug_list'], interaction['item_id_aug_length']

...
class ADARecTrainer(Trainer):
    def __init__(self, config, model):
        super(ADARecTrainer, self).__init__(config, model)
        self.optimizer_e5 = self._build_optimizer(params=self.model.expert_e5.parameters())
        self.optimizer_e10 = self._build_optimizer(params=self.model.expert_e10.parameters())

    def _train_one_expert(self, interaction, expert_name, optimizer):
        optimizer.zero_grad()
        loss = self.model.calculate_expert_loss(interaction, expert_name)
        self._check_nan(loss)
        loss.backward()
        if self.clip_grad_norm:
            expert = self.model.expert_e5 if expert_name == "e5" else self.model.expert_e10
            clip_grad_norm_(expert.parameters(), **self.clip_grad_norm)
        optimizer.step()
        return loss.item()
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for interaction in iter_data:
            interaction = interaction.to(self.device)
            total_loss_e5 += self._train_one_expert(
                interaction, "e5", self.optimizer_e5
            )
            total_loss_e10 += self._train_one_expert(
                interaction, "e10", self.optimizer_e10
            )
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(
                    set_color("GPU RAM: " + get_gpu_usage(self.device), "yellow")
                )
        return total_loss_e5, total_loss_e10
    ...
    def _save_checkpoint(self, epoch, verbose=True, **kwargs):
        ...
        state = {
            ...
            "state_dict": self.model.state_dict(),
            "other_parameter": self.model.other_parameter(),
            "optimizer_e5": self.optimizer_e5.state_dict(),
            "optimizer_e10": self.optimizer_e10.state_dict(),
        }
        torch.save(state, saved_model_file, pickle_protocol=4)
        if verbose:
            self.logger.info(
...
class CoSeRecTrainer(Trainer):
    ...
    def __refresh_training_dataset(self, item_embeddings):
        ...
        """
        self.model.online_similarity_model.update_embedding_matrix(item_embeddings)

    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        if epoch_idx > self.args.augmentation_warm_up_epoches:
            # print("refresh dataset with updated item embedding")
            self.__refresh_training_dataset(self.model.item_embedding)
...
class ICLRecTrainer(Trainer):
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        if epoch_idx >= self.args.warm_up_epoches:
        ...
        if epoch_idx >= self.args.warm_up_epoches:
            ...
            # train cluster
            # self.logger.info("Training Clusters:")
            self.model.cluster.train(kmeans_training_data)
            # clean memory
...
class ICSRecTrainer(Trainer):
    def __init__(self, config, model):
        super(ICSRecTrainer, self).__init__(config, model)
        self.args = config
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        if self.args.cl_mode in ['cf','f']:
            ...
            for batch_idx, interaction in enumerate(iter_data):
                ...
                """
                interaction = interaction.to(self.device)
                subsequence = interaction[self.model.ITEM_SEQ]
                subsequence_lengths = interaction[self.model.ITEM_SEQ_LEN]
                sequence_output = self.model(subsequence, subsequence_lengths) # [BxH]
                kmeans_training_data.append(sequence_output.detach().cpu().numpy())

            kmeans_training_data = np.concatenate(kmeans_training_data, axis=0)
            kmeans_training_data_t = [kmeans_training_data]

            for i, clusters in tqdm(enumerate(self.model.clusters_t), total=len(self.model.clusters_t)):
                for j, cluster in enumerate(clusters):
                    cluster.train(kmeans_training_data_t[i])
                    self.model.clusters_t[i][j] = cluster
